...
if [[ `/bin/ps aux | /bin/egrep -i "[0-9]{1,2}:[0-9]{1,2} /usr/bin/python /usr/local/bin/motiondetection.py" | wc -l` < 1 ]]; then 
    ...
    /usr/bin/python /usr/local/bin/motiondetection.py -e 'sshmonitorapp@gmail.com' -p 'hkeyscwhgxjzafvj' &
elif [[ `/bin/ps aux | /bin/egrep -i "[0-9]{1,2}:[0-9]{1,2} /usr/bin/python /usr/local/bin/motiondetection.py" | wc -l` < 1 ]]; then 

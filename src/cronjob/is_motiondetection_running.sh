...
if [[ `/bin/ps aux | /bin/egrep -i "[0-9]{1,2}:[0-9]{1,2} /usr/bin/python3 /usr/local/bin/motiondetection.py" | wc -l` < 1 ]]; then 
    ...
    /usr/bin/sudo LD_PRELOAD=/usr/arm-linux-gnueabihf/lib/libatomic.so.1.2.0 /usr/bin/python3 /usr/local/bin/motiondetection.py -e 'sshmonitorapp@gmail.com' -p 'hkeyscwhgxjzafvj' --verbose --burst-mode=5 &
